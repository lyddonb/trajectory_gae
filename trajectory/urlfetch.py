...
# Whether the trajectory urlfetch hook is installed.
installed = False
...
def _install_trajectory(config):
    ...
    def check(service, name, request, response):
        """Called before hitting the urlfetch stub."""
        assert service == STUBS.URLFETCH
        ...
        try:
            ...
            logging.info("Unable to apply trajectory to urlfetch request.", e)
...
def install_hook(config):
    ...
    apiproxy.GetPreCallHooks().Append(
        'trajectory_urlfetch_hook', _install_trajectory(config),
        STUBS.URLFETCH)
